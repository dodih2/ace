...
class Absen_dosen_model extends CI_Model{
  ...
  function get_all_absen(){
    ...
    $this->datatables->where('konfirmasi', 1);
    $this->datatables->where("(nama_hari = '$date' AND NOW() BETWEEN jam_mulai AND jam_selesai)");
    $this->datatables->add_column('view','<a href="javascript:void(0);" class="edit_record btn btn-info btn-xs" data-nim="$1" data-nama="$2" data-kelas="$3">Absen</a>','nim, nama, kelas, kelas_id, nik_id, nama_hari, kelas_nama,konfirmasi');
    return $this->datatables->generate();
  }

  function get_all_absenn(){ //ambil data mahasiswa dari tabel dari tabel user_mahasiswa
    $date = date('D');
    $this->datatables->select('id_absen, nik_id_id, hadir, alpa, izin, ket_telat, keterangan, konfirmasi, tanggal, kelas, jam_mulai, jam_selesai, nama_hari, kelas_id, kelas_nama, nim, nama');
    $this->datatables->from('absen');
    ...
    $this->datatables->join('user_mahasiswa','absen.nim_id=user_mahasiswa.nim');
    $this->datatables->where("(nama_hari = '$date' AND NOW() BETWEEN jam_mulai AND jam_selesai)");
    $this->datatables->add_column('view','<a href="javascript:void(0);" class="edit_absen btn btn-info btn-xs" data-id_absen="$1" data-nik="$2" data-keterangan="$7">Sunting</a>','id_absen, nik_id_id, hadir, alpa, izin, ket_telat, keterangan, konfirmasi, tanggal, kelas, jam_mulai, jam_selesai, nama_hari, kelas_id, kelas_nama, nim, nama');
    return $this->datatables->generate();
  }
